...
def generate_prompt(data_point):
    ...
    text = (
            f"<start_of_turn>user {prefix_text} {data_point['text']} "
            f"<start_of_turn>model entities: {data_point['entities']} relations: {data_point['relations']}<end_of_turn>"
        )

    return text

text_column = [generate_prompt(data_point) for data_point in dataset['train']]
dataset['train'] = dataset['train'].add_column("prompt", text_column)
train_data = dataset['train'].to_pandas()

text_column = [generate_prompt(data_point) for data_point in dataset['test']]
dataset['test'] = dataset['test'].add_column("prompt", text_column)
...
merged_model.to("cpu")
tokenizer.save_pretrained("gemma_trained")
merged_model.save_pretrained("gemma_trained",safe_serialization=True)
tokenizer.pad_token = tokenizer.eos_token
